...
public class UserPrefSession implements UserPrefLocal {
	...
	public UserPrefProjectWorkflow updateProjectWoPref(UserPrefProjectWorkflow projectWorkflow) {
		UserPrefProjectWorkflow result = null;
		
		try {
			result = em.merge(projectWorkflow);
		} catch (EJBException ex) {
			logger.error(ex);
			//throw new ProcessException(ex.getMessage());
		}
	...
	public UserPrefProjectWorkflow getProjectWorkFlowPrefByUserId(Integer userID) {
		...
		Query query = em.createNamedQuery("UserPrefProjectWorkflow.findProjectWorkFlowPrefByUserId");
		query.setParameter("userId", userID);

		try {
			result = (UserPrefProjectWorkflow) query.getSingleResult();
		} catch (NoResultException ex) {
			logger.error(ex);
			return null;
		}
